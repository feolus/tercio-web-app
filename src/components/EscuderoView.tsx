...
const EscuderoView: React.FC = () => {
    const { currentUser, updateUser } = useContext(UserContext);
    const { masterTroops } = useContext(DataContext);
    const [activeTab, setActiveTab] = useState<'hero' | 'troops'>('hero');
    ...
    const tabClass = (tabName: 'hero' | 'troops') => {
            ...
            <div className="border-b border-slate-200">
                <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                    ...
                    <button
                        ...
                        className={tabClass('troops')}
                        aria-current={activeTab === 'troops' ? 'page' : undefined}
                    >
                        Mis Tropas
                    </button>
                </nav>
